...
func (c *Client) CreateTournamentCode(region string, tournamentID int64, count *int, params TournamentCodeParametersV5) ([]string, error) {
	...
	var codes []string
	if err := c.post(region, "/lol/tournament-stub/v5/codes?"+query.Encode(), params, &codes); err != nil {
		return nil, err
	}
	return codes, nil
}
...
func (c *Client) GetTournamentCode(region, tournamentCode string) (*TournamentCodeV5DTO, error) {
	...
	if err := c.get(region, "/lol/tournament-stub/v5/codes/"+url.PathEscape(tournamentCode), &code); err != nil {
		return nil, err
	}
	return &code, nil
}
...
func (c *Client) GetLobbyEventsByCode(region, tournamentCode string) (*LobbyEventV5DTOWrapper, error) {
	...
	if err := c.get(region, "/lol/tournament-stub/v5/lobby-events/by-code/"+url.PathEscape(tournamentCode), &events); err != nil {
		return nil, err
	}
	return &events, nil
}
...
func (c *Client) RegisterProviderData(region string, params ProviderRegistrationParametersV5) (int, error) {
	var providerID int
	if err := c.post(region, "/lol/tournament-stub/v5/providers", params, &providerID); err != nil {
		return 0, err
	}
...
func (c *Client) RegisterTournament(region string, params TournamentRegistrationParametersV5) (int, error) {
	...
	if err := c.post(region, "/lol/tournament-stub/v5/tournaments", params, &tournamentID); err != nil {
		return 0, err
	}
